...
DogHitCat_Slave proc
	...
	; determine direction of the hit
	                       mov  bx , xd
	                       cmp  bx ,xCoord                        	; compare xdog with x cat
	                       ja   decrease_Xball_slave              	; if Xdog > Xcat decrease ball position x
	                       mov  cx,6                              	; else increase ball position x
	                       add  xball , cx
	
	continue_draw2_slave:  MOV  VALUE,0FAH
	                       CALL SENDVALUE
	                       mov  bx, xball
						  
	; store ball new position x in bx
	;	mov dx, xCoord ; postion of colliosion (X Dog-10)
	;	sub dx,10 ; stop point
	;	push dx  ;  save point of colliosion on stack
	                       Call delay
	...
	;call delay2
	                       ...
	                       JMP  repeat2_slave
	decrease_Xball_slave:  
	                       mov  cx,6
	                       sub  xball , cx
	                       jmp  continue_draw2_slave

	finish2_slave:         MOV  VALUE,0
	                       MOV  VALUER,0
	                       MOV  VALUER2,0
	                       mov  start_balling ,0
	                       mov  Cat_Took_Shield, 0
	                       ret
	NEWHEALTHDOG:          MOV  VALUE,0Ah
	                       CALL SENDVALUE
	                       CALL RECEIVEVALUEEE
	                       CMP  VALUER2,0
	                       JZ   NODECHEALTHDOG
	...
	DECTHEHEALTHDOG:       
	                       ...
	                       call Draw_Health_Bar
	                       LOOP DECTHEHEALTHDOG
	                       MOV  VALUER2,0
	NODECHEALTHDOG:        JMP  finish2_slave

	;;;;;;;;;;;;;;;;; hit effect decrease health of the dog  ;;;;;;;;;;;;;;;;
	
DogHitCat_slave endp
...
CatHitDog_slave proc
	...
	; determine direction of the hit
	                       mov  bx , xCoord
	                       cmp  bx ,xd                            	; compare xdog with x cat
	                       jb   increase_Xfish_slave              	;RIGHT: if Xcat < Xdog increase fish position x
	                       mov  cx,6
	                       sub  xf , cx                           	;decrease fish position x
	continue_draw_slave:   MOV  VALUE, 0FAH
	                       CALL SENDVALUE
	                       mov  bx, xf                            	; store fish position x in bx
	;  sub bx, fish_W
	                       mov  dx, xd                            	; postion of colliosion (X Dog-10)
	...
	;call delay2
	                       ...
	                       CALL SENDVALUE
	                       JMP  repeat_slave
	increase_Xfish_slave:  mov  cx,6
	                       add  xf , cx
	                       jmp  continue_draw_slave
	finish_slave:          
	                       MOV  VALUER,0
	                       MOV  VALUER2,0
	                       mov  start_hitting ,0
...
read_the_key endp
read_the_key_slave proc
	                       mov  isJumping , 0
	CHECK_slave:           mov  ah,1
	                       int  16h
	                       jz   reyooo_slave
	                       mov  ah,0
	                       int  16h
	...
	ReadKey_slave:         
	                       ...
	                       jmp  CHECK_slave
	;;;;;;;;;;;;;;;;;;;;;; temp lables ;;;;;;;;;;;;;;;;;;;
	reyooo_slave:          cmp  start_balling ,1
	                       JZ   TEMPYOO_slave
	                       CMP  start_hitting,1
	                       JZ   TEMPYOO_slave
	                       CALL RECEIVEVALUE
	                       ...
	                       JG   movecat_slave
	                       JMP  yooo_slave
	reyooo2_slave:         CALL RECEIVEVALUEEE
	                       CMP  VALUER2,0
	                       JG   movedog_slave
	                       JMP  yooo_slave
	
	HITTHEBALL_slave:      
	                       call DogHitCat_slave
	                       jmp  ReadKey_slave

	HITTHEFISH_slave:      call CatHitDog_slave
	                       jmp  ReadKey_slave
	
	TEMP_READKEY_slave:    JMP  ReadKey_slave
	;;;;;;;;;;;;;;;;;;;;;;;;;;; CAT MOVEMENT LABLES ;;;;;;;;;;;;;;;;;;;;;;;;;;;

	
	TEMPYOO_slave:         JMP  yooo_slave
	;;;;;;;;;;;;;;;;;;;;;; DOG MOVEMENT LABLES ;;;;;;;;;;;;;;;;;;;;;
	movedog_slave:         MOV  VALUE , 0Fh
	                       CALL SENDVALUE
	                       MOV  BX , VALUER2
	                       MOV  xd ,BX
	                       CALL RECEIVEVALUEEE
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;; TO READ KEY WHILE HITTING ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	kamel_darb_slave:      cmp  start_hitting ,1
	                       je   bye_slave
	                       jmp  ReadKey_slave

	kamel_ball_slave:      cmp  start_balling ,1
	                       je   bye_slave
	                       jmp  ReadKey_slave
	;;;;;;;;;;;;;;;;;;;;;;;;;;;to check if key not pressd while hitting or not ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

	yooo_slave:            cmp  start_hitting , 1
	                       je   bye_slave
	                       cmp  start_balling , 1
	                       je   bye_slave
	                       mov  ah,1
